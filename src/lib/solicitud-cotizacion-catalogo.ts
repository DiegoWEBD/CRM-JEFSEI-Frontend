...
export type CampoSolicitudCotizacionTipo =
	| 'text'
	| 'number'
	| 'actividades_aseguradas'
...
export const TIPO_CLIENTE_SOLICITUD_LABELS: Record<
...
> = {
	persona_natural: [
		...
		{
			key: 'salud_complementario',
			label: 'Seguro salud / complementario',
			campos: [],
		},
...
export function camposSolicitudParaLinea(
	...
	if (!lineaKey) return []
	return (
		LINEAS_SOLICITUD_POR_TIPO[tipo].find(l => l.key === lineaKey)?.campos ?? []
	)
}
...
export function lineaUsaActividadesAseguradas(
	...
	return camposSolicitudParaLinea(tipo, lineaKey).some(
...
export function inferirTipoClienteSolicitud(
	...
	return lineaNegocioNombre.trim().toLowerCase() === 'condominio'
		? 'condominio'
		: 'persona_natural'
}
