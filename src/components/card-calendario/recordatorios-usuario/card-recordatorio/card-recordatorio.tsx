...
export default function CardRecordatorio({
	...
	const badgePrincipal = esRenovacion
		? { label: `PÃ³liza ${recordatorio.numero_poliza}`, className: 'h-5 max-w-full truncate px-1.5 text-xs font-normal border-sky-500/25 bg-sky-500/10 text-sky-950 dark:text-sky-100' }
		: esCobranza
			? { label: `Cobranza â€” PÃ³liza ${recordatorio.numero_poliza}`, href: `/polizas/${recordatorio.numero_poliza}`, className: 'h-5 max-w-full truncate px-1.5 text-xs font-normal border-amber-500/25 bg-amber-500/10 text-amber-950 dark:text-amber-100' }
			: asociado
				? { label: recordatorio.nombre_prospecto?.trim() ?? 'â€”', href: `/prospectos/${recordatorio.id_prospecto}`, className: 'h-5 max-w-full truncate px-1.5 text-xs font-normal' }
	...
	return (
		...
		<div className='rounded-md border border-border bg-secondary/25 px-2.5 py-2'>
			...
			<p className='mt-1 text-xs leading-tight text-muted-foreground'>
				{etiquetaTipoRecordatorio(recordatorio.tipo_gestion)} Â·{' '}
				{formatearFecha(
			...
			<div className='mt-1.5 flex flex-wrap gap-1'>
				...
				>
					{isCompletando ? 'Completandoâ€¦' : 'Completar'}
				</Button>
				<>
				<Button
					size='sm'
				...
				>
					...
					>
						{isEliminando ? 'Eliminandoâ€¦' : 'Eliminar'}
					</Button>
				</>
			</div>
		</div>
