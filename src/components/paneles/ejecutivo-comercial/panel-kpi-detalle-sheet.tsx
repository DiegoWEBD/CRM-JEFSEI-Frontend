...
import { KpiPanelDetalleKey } from '@/types/ejecutivo-comercial/panel/kpi-panel-detalle-key'
				...
				<ScrollArea className='flex-1 px-4 py-3'>
					{!datos ? (
						...
						<TablaClientes
							...
							destacarNuevos={datos.destacarNuevos}
						/>
					...
					{kpiKey === 'cotiz' ? (
						<Button size='sm' variant='outline' className='h-8 text-xs' asChild>
							<Link href='/solicitudes-estudio'>
								Ir a solicitudes de cotización
							</Link>
						</Button>
					) : null}
					{kpiKey === 'estDisp' ? (
						<Button size='sm' variant='outline' className='h-8 text-xs' asChild>
							<Link href='/cotizaciones-estudios-emitidos'>
								Ir a estudios emitidos
							</Link>
						</Button>
					) : null}
				</div>
			...
			<Table>
				<TableHeader>
					<TableRow className='hover:bg-transparent'>
						...
						<TableHead className='text-right text-[10px] uppercase text-muted-foreground'>
							Acción
						</TableHead>
					</TableRow>
				</TableHeader>
						...
						>
							<TableCell>
								...
								{destacarNuevos && c.asignacionPendienteRevision ? (
									<Badge
										variant='outline'
										className='mt-1 border-amber-500/40 text-[9px] text-amber-900'
									>
										Nuevo por revisar
									</Badge>
								) : null}
							...
							<TableCell>
								<Badge
									...
									className={cn(
										'text-[9px]',
										ESTADO_CLIENTE_COMERCIAL_BADGE[c.estadoComercial],
									)}
								>
							...
							<TableCell className='text-right'>
								<Button
									...
									asChild
								>
									<EnlacePerfilEjecutivoComercial clienteId={c.id}>
										Ver
...
function TablaSolicitudes({
	...
	solicitudes: SolicitudEstudioRowMock[]
}) {
	if (solicitudes.length === 0) {
		return (
			<p className='text-xs text-muted-foreground'>
				Sin cotizaciones solicitadas.
			</p>
		)
	}
	return (
							...
							<TableRow key={s.id} className='text-xs'>
								...
								<TableCell>
									<Badge
										...
										className={cn(
											'text-[9px]',
											ESTADO_SOLICITUD_COMERCIAL_BADGE[label],
										)}
									>
										{label}
									</Badge>
...
function TablaEstudiosDisponibles({
	estudios,
}: {
	estudios: EstudioDisponiblePanelRow[]
}) {
	if (estudios.length === 0) {
		return (
			<p className='text-xs text-muted-foreground'>Sin estudios disponibles.</p>
		)
	}
	return (
		<div className='overflow-x-auto rounded-md border border-border'>
