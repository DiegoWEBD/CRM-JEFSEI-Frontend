...
export default function DialogNuevaSolicitudCotizacion({
	...
	const mutationProceso = useSolicitarCotizacionProceso(
		idProceso ?? 0,
		idProspecto,
	)
	...
	function transformarARequest(
		...
		const base = {
			...
			monto_asegurado: values.monto_asegurado
				? Number(values.monto_asegurado)
				: null,
		}
	...
	function reiniciarCamposDinamicos(
	...
	const lineaActual: LineaSeguroSolicitudKey | '' =
		(tipoPredefinido as LineaSeguroSolicitudKey) ?? formik.values.lineaSeguro
	const camposDinamicos = camposSolicitudParaLinea(tipoCliente, lineaActual)
	const camposEstandar = camposDinamicos.filter(
		c => c.tipo !== 'actividades_aseguradas',
	)
	const esAccidentesPersonales = lineaUsaActividadesAseguradas(
		tipoCliente,
		lineaActual,
	)
	...
	function handleAddActividad() {
		formik.setFieldValue('actividades', [
	...
	function handleRemoveActividad(index: number) {
		...
		formik.setFieldValue(
			'actividades',
			nuevas.length === 0 ? [nuevaActividadFila()] : nuevas,
		)
	}
				...
				<form onSubmit={formik.handleSubmit} className='space-y-3'>
								...
								{formik.values.actividades.map((act, index) => (
									<div key={index} className='flex items-start gap-2'>
										<div className='flex flex-1 flex-col gap-1'>
											...
											/>
											{formik.errors[
												`actividades.${index}.actividad` as keyof typeof formik.errors
											] && (
										...
										<div className='flex flex-1 flex-col gap-1'>
											<Input
												...
												onChange={e =>
													formik.setFieldValue(
														`actividades.${index}.numero_asegurados`,
														e.target.value,
													)
												}
											/>
											{formik.errors[
												`actividades.${index}.numero_asegurados` as keyof typeof formik.errors
											] && (
												<p className='text-xs text-destructive'>
													{
														formik.errors[
					...
					{camposEstandar.length > 0 ? (
						<div className='space-y-3 rounded-md border border-border/70 bg-muted/10 p-3'>
							...
							{camposEstandar.map(campo => {
								const fieldKey =
									campo.key as keyof DialogNuevaSolicitudFormValues
								return (
					...
					<DialogFooter className='gap-2'>
						...
						<Button
							...
							disabled={
								formik.isSubmitting ||
								(!tipoPredefinido && !formik.values.lineaSeguro)
							}
						>
							{formik.isSubmitting ? (
